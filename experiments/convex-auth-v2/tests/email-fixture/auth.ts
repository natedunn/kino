...
import { sha256Hex } from '../../.upstream/packages/core/src/lib/crypto.ts';
import { internalMutation } from './_generated/server';
import { callbacks, components } from './refs';

// Provider-authoring experiment only. Missing recovery revocation means this
// fixture must not be installed into a real application.
...
export const onSignIn = internalMutation({
	...
	handler: async (ctx, { userId }) => {
		...
		if (!user?.verified || user.rejectSignIn) throw new ConvexError('SIGN_IN_REJECTED');
		return null;
	},
});
...
async function challenge(ctx: MutationCtx, code: string, purpose: 'verify' | 'reset') {
	...
	if (!user || proof.generation !== generation || (purpose === 'verify' && user.verified))
		throw new ConvexError('INVALID_PROOF');
	return { proof, user };
}
...
export const resetPassword = authMutation({
	...
	handler: async (ctx, args) => completeReset(ctx as MutationCtx, args, false),
});
...
async function completeReset(
	...
	if (user.rejectResetCompletion) throw new ConvexError('RESET_COMPLETION_REJECTED');
