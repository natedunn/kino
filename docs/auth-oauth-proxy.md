...
The originally observed broken behavior was:
...
PR #14: https://github.com/natedunn/kino/pull/14

PR #15: https://github.com/natedunn/kino/pull/15

PR #14 added the auth proxy and SSR fixes. PR #15 addressed review follow-ups around forwarded-host security, helper initialization, and SSR Convex client reuse.
...
- Validated forwarded OAuth origins against trusted origins before rewriting Better Auth's request URL.
- Ignored generic `x-forwarded-*` headers for OAuth origin rewriting to reduce spoofing risk.
...
- Made loopback wildcard host derivation explicit.
...
- Lazily reused the TanStack Start Better Auth helper instance.
- Reused SSR `ConvexHttpClient` instances per request `QueryClient` and `convexUrl + token` pair.
- Kept client-side loader `ensureQueryData` on Kitcn's installed default `ConvexQueryClient.queryFn()`.
...
- Production must run the OAuth proxy-compatible code. Preview auth cannot fully work if only preview has the auth proxy changes deployed.
...
2. Confirm preview deploy is also on the same main code.
...
1. Production is actually deployed with the merged auth proxy code.
