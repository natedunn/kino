// Shared GitHub transport for native Relay actions.
import { getEnv, getGitHubRelayEnv, isTrustedOrigin } from './relayEnv';
import { RelayError as CRPCError } from './relayError';
...
export function sanitizeGitHubRepository(repository: GitHubRepository): GitHubRepository {
	return {
...
export function getRequiredGitHubRelayEnv(): GitHubRelayEnv {
	...
	return {
		...
		webhookSecret: assertString(env.webhookSecret, 'GITHUB_RELAY_WEBHOOK_SECRET'),
	};
}
...
function rsaPrivateKeyToPkcs8Der(rsaPrivateKeyDer: Uint8Array) {
	...
	return derSequence(version, rsaEncryptionAlgorithmIdentifier, derOctetString(rsaPrivateKeyDer));
}
...
export function resolveGitHubCallbackTargetUrl(requestedTargetUrl?: string) {
	const env = getRequiredGitHubRelayEnv();
	const targetUrl = env.callbackTargetUrl ?? requestedTargetUrl ?? getGitHubCallbackTargetUrl();
...
export async function createGitHubAppState(payload: Omit<GitHubAppStatePayload, 'v'>) {
	const env = getRequiredGitHubRelayEnv();
...
async function githubFetch<T>(url: string, init: RequestInit & { token?: string }) {
...
export async function listUserInstallations(userToken: string) {
	...
	if (result.total_count > 100)
		throw new CRPCError({
			code: 'CONFLICT',
			message: 'Too many GitHub installations to refresh safely. Contact support.',
		});
...
export async function listInstallationRepositories(token: string) {
	...
	for (let page = 1; page <= 5; page++) {
		...
		if (result.total_count > 500)
			throw new CRPCError({
				code: 'CONFLICT',
				message: 'Too many GitHub repositories to refresh safely. Contact support.',
			});
...
export async function searchRepositoryIssues(args: {
	...
	url.searchParams.set('q', `repo:${args.repository.full_name} is:issue ${search}`);
