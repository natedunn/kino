...
describe("client/server validation parity", () => {
  ...
  it("keeps every shared length limit identical", () => {
    ...
    for (const key of Object.keys(FORM_LIMITS) as Array<
      keyof typeof FORM_LIMITS
    >) {
      expect(
        VALIDATION_LIMITS,
      ...
      expect(
        FORM_LIMITS[key],
        `length limit "${key}" drifted between client and server`
      ).toBe(VALIDATION_LIMITS[key as keyof typeof VALIDATION_LIMITS])
    }
  })
