...
describe("slug validation", () => {
  ...
  it("applies separate caps for org, project, and generated slugs", () => {
    expect(
      orgSlugSchema.safeParse("a".repeat(VALIDATION_LIMITS.orgSlug)).success
    ).toBe(true)
    expect(
    ...
    expect(
      projectSlugSchema.safeParse("a".repeat(VALIDATION_LIMITS.projectSlug))
        .success
    ).toBe(true)
    expect(
      projectSlugSchema.safeParse("a".repeat(VALIDATION_LIMITS.projectSlug + 1))
        .success
    ).toBe(false)

    expect(
      generatedSlugSchema.safeParse("a".repeat(VALIDATION_LIMITS.generatedSlug))
        .success
    ).toBe(true)
    expect(
      generatedSlugSchema.safeParse(
        "a".repeat(VALIDATION_LIMITS.generatedSlug + 1)
      ).success
    ).toBe(false)
  })

  it("normalizes generated slugs with the requested cap", () => {
    expect(normalizeSlug("  A Slug!! With___Noise  ", 20)).toBe(
      "a-slug-withnoise"
    )
...
describe("URL and email validation", () => {
  it("only accepts http and https URLs", () => {
    ...
    expect(httpUrlSchema.safeParse("ftp://example.com/file").success).toBe(
      false
    )
  })
