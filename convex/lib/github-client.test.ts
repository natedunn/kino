...
describe('github helpers', () => {
	it('parses GitHub App private keys with RSA and PKCS#8 PEM wrappers', () => {
		...
		expect(rsaWrapped).toEqual([
			...
			0x01, 0x01, 0x01, 0x05, 0x00, 0x04, 0x04, 0x01, 0x02, 0x03, 0x04,
		]);

		expect(
			Array.from(
				new Uint8Array(
					privateKeyToDer(`-----BEGIN PRIVATE KEY-----\n${base64Der}\n-----END PRIVATE KEY-----`)
				)
			)
		).toEqual([1, 2, 3, 4]);
	});
	...
	it('trusts callback targets through the shared origin policy', () => {
		...
		expect(isTrustedGitHubCallbackTarget('https://usekino.com/api/github/callback')).toBe(true);
		expect(
			isTrustedGitHubCallbackTarget(
		...
		process.env.TRUSTED_ORIGINS = 'https://brainy-boar-871.convex.site,https://staging.usekino.com';

		expect(
			isTrustedGitHubCallbackTarget('https://brainy-boar-871.convex.site/api/github/callback')
		).toBe(true);
		expect(isTrustedGitHubCallbackTarget('https://staging.usekino.com/api/github/callback')).toBe(
			true
		);
	});
	...
	it('trusts local callback targets only when local origins are trusted', () => {
		...
		expect(
			isTrustedGitHubCallbackTarget('https://mizar.kino.localhost:1355/api/github/callback')
		).toBe(false);
		...
		expect(isTrustedGitHubCallbackTarget('http://127.0.0.1:5173/api/github/callback')).toBe(true);
	});

	it('rejects callback targets with untrusted paths or protocols', () => {
		setGitHubRelayEnv();

		expect(isTrustedGitHubCallbackTarget('https://usekino.com/auth')).toBe(false);
		expect(isTrustedGitHubCallbackTarget('ftp://usekino.com/api/github/callback')).toBe(false);
	});
	...
	it('uses a trusted requested callback target URL when no override is configured', () => {
		...
		expect(
			resolveGitHubCallbackTargetUrl('https://mizar.kino.localhost:1355/api/github/callback')
	...
	it('rejects untrusted requested callback targets', () => {
		...
		expect(() =>
			resolveGitHubCallbackTargetUrl('https://evil.example.com/api/github/callback')
...
describe('verifyGitHubWebhookSignature', () => {
	...
	const VALID_SIG = 'sha256=b79f7f179b41184d008131377978dead58052349a984dd3e0352c64488989813';
	...
	it('rejects a tampered body', async () => {
		setGitHubRelayEnv();
		expect(await verifyGitHubWebhookSignature('{"action":"pong"}', VALID_SIG)).toBe(false);
	});
	...
	it('rejects missing or unprefixed signature headers', async () => {
		...
		expect(await verifyGitHubWebhookSignature(BODY, VALID_SIG.replace('sha256=', ''))).toBe(false);
		expect(await verifyGitHubWebhookSignature(BODY, VALID_SIG.replace('sha256=', 'sha1='))).toBe(
			false
		);
	});
