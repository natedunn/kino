...
export const DEFAULT_FEEDBACK_BOARDS = [
  "Bugs",
  "Feature Requests",
  "Improvements",
] as const
 ...
 * unexpected value.
 */
export function sanitizeSystemRole(
...
export async function ensureUniqueUsername(ctx: OrmCtx, preferred: string) {
  const base =
    slugify(preferred).replace(/-/g, "_").slice(0, 30) ||
    `user_${generateRandomSuffix()}`
  let candidate = base
  let attempt = 0
  while (attempt < 10) {
...
export async function ensureUniqueOrgSlug(ctx: OrmCtx, preferred: string) {
  ...
  while (attempt < 10) {
    ...
    if (!existing) return candidate
    candidate = normalizeSlug(
      `${base.slice(0, max - suffixWidth)}-${generateRandomSuffix()}`,
      max
    )
    attempt++
  }
...
export async function getCurrentProfileOrThrow(
  ctx: OrmCtx,
  userId: string | null | undefined
) {
...
export async function setUserProfileId(
  ctx: OrmMutationCtx,
  userId: string,
  profileId: string
) {
...
export async function ensureUserBootstrap(
  ...
  if (legacyUserId) {
    ...
    await Promise.all(
      legacyMemberships.flatMap((membership: any) => {
        ...
        return membershipId
          ? [ctx.db.patch(membershipId as any, { userId: publicUserId })]
          : []
      })
    )
  }
  ...
  if (memberships.length === 0) {
    ...
    const inferredPersonalOrganizationId = await inferPersonalOrganizationId(
      ctx,
      {
