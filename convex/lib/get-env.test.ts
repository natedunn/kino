...
describe('trusted auth origins', () => {
	...
	it('reads the OAuth proxy production URL (the tier gateway)', () => {
		...
		expect(getOAuthProxyProductionUrlEnv()).toBe('https://gateway-dev.usekino.com');
	});
	...
	it('extracts trusted forwarded auth origins for Better Auth base URL resolution', () => {
		...
		expect(
			getTrustedForwardedAuthOrigin(
				new Request('https://scrupulous-lemming-700.convex.site/api/auth/sign-in/social', {
					headers: {
						'x-better-auth-forwarded-host': 'local-dev-portless-worktrees.kino.localhost:1355',
