...
async function seedUser(
  ctx: Ctx,
  args: { email: string; name: string; username: string }
) {
...
describe("user data export", () => {
  ...
  it("exports only the signed-in user's comments with visible context", async () => {
    ...
    expect(result.sections.comments.feedbackComments[0].content).toBe(
      "Please fix this"
    )
    ...
    expect(result.sections.comments.updateComments[0].content).toBe(
      "Looks good"
    )
