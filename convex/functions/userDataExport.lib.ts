...
export async function getVisibleOrganizationSummary(ctx: ExportCtx, args: { orgSlug: string }) {
...
export async function getFeedbackCommentContext(ctx: ExportCtx, comment: Doc<'feedbackComment'>) {
...
export async function getUpdateCommentContext(ctx: ExportCtx, comment: Doc<'updateComment'>) {
...
export async function buildCommentsSection({
	...
	const [feedbackComments, updateComments] = await Promise.all([
		ctx.db
			...
			.withIndex('by_authorProfileId', (q: any) => q.eq('authorProfileId', profile._id))
			.order('asc')
