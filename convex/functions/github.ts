...
import { authMutation, authQuery, privateMutation, privateQuery } from '../lib/crpc';
import {
...
export const startProjectConnection = authMutation
	...
	.mutation(async ({ ctx, input }) => {
		const { organization, profile, project } = await verifyOrgAdminForProject(ctx, {
	...
	.query(async ({ ctx, input }) => {
		...
		const installation = await ctx.db
			...
			.withIndex('by_orgId_installationId', (q: any) =>
				q.eq('orgId', access.organization.id).eq('installationId', input.installationId)
			)
			.unique();
...
export const completeInstallationCallback = privateMutation
	...
	.mutation(async ({ ctx, input }) => {
		...
		const values = {
			...
			status: input.setupAction === 'remove' ? ('deleted' as const) : ('active' as const),
			updatedTime: Date.now(),
		};
...
export const saveRepositoryConnection = privateMutation
	...
	.mutation(async ({ ctx, input }) => {
		...
		const installation = await ctx.db
			.query('githubInstallation')
			.withIndex('by_orgId_installationId', (q: any) =>
				q.eq('orgId', organization.id).eq('installationId', input.installationId)
			)
			.unique();
		if (!installation || installation.status !== 'active') {
			throw new CRPCError({
		...
		await Promise.all(
			projectConnections
				.filter(
					(connection: any) => !connection.deletedTime && connection._id !== existingForRepo?._id
				)
				.map((connection: any) =>
					ctx.orm
...
export const disconnectRepository = authMutation
	...
	.mutation(async ({ ctx, input }) => {
		...
		const connection = connections.find(
			(item: any) =>
				item._id === input.connectionId && item.orgId === organization.id && !item.deletedTime
		);
...
export const processWebhookEvent = privateMutation
	...
	.mutation(async ({ ctx, input }) => {
		...
		if (input.event === 'installation' && input.installation) {
			const installations = await ctx.db
				.query('githubInstallation')
				.withIndex('by_installationId', (q: any) => q.eq('installationId', input.installation!.id))
				.take(50);
					...
					: input.action === 'suspend'
						...
						: input.action === 'unsuspend'
							...
							: input.action === 'new_permissions_accepted'
								? {
										...(input.installation.events ? { events: input.installation.events } : {}),
		...
		if ((input.event === 'issues' || input.event === 'issue_comment') && input.issue) {
			const repositoryConnections = await ctx.db
				.query('githubRepositoryConnection')
				.withIndex('by_repoId', (q: any) => q.eq('repoId', input.issue!.repositoryId))
				.take(50);
