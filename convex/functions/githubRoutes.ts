...
export const webhook = publicRoute.post('/api/github/webhook').mutation(async ({ ctx, c }) => {
	const body = await c.req.text();
	const verified = await verifyGitHubWebhookSignature(body, c.req.header('x-hub-signature-256'));
	if (!verified) {
		throw new CRPCError({
			code: 'UNAUTHORIZED',
	...
	const deliveryId = webhookDeliveryIdSchema.safeParse(c.req.header('x-github-delivery'));
	...
	const result = await caller.processWebhookEvent({
		action:
			typeof payload.action === 'string' ? webhookActionSchema.parse(payload.action) : undefined,
