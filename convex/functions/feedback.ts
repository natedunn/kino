...
export const remove = authMutation
	...
	.mutation(async ({ ctx, input }) => {
		const { feedback, permissions } = await verifyFeedbackWriteAccess(ctx, input.id, ctx.userId);
		assertCanAdminFeedback(permissions);

		await ctx.orm.delete(feedbackTable).where(eq(feedbackTable.id, feedback._id));
...
export const updateAssigned = authMutation
	...
	.mutation(async ({ ctx, input }) => {
		...
		} else if (input.assignedProfileId && input.assignedProfileId !== oldAssignedProfileId) {
			await recordFeedbackEvent(ctx, {
				actorProfileId: profile._id,
				eventType: 'assigned',
				feedbackId: feedback._id,
...
export const listProjectFeedback = optionalAuthQuery
	...
	.query(async ({ ctx, input }) => {
		...
		if (input.search?.trim()) {
			query = ctx.db
				.query('feedback')
				.withSearchIndex('by_projectId_boardId_status_searchContent', (builder: any) => {
					let next = builder
			...
			query = ctx.db
				.query('feedback')
				.withIndex('by_projectId', (q: any) => q.eq('projectId', asId<'project'>(input.projectId)))
				.order(input.order ?? 'desc');
		}
		...
		return {
			...
			page: await Promise.all(
				page.map(async (item: any) => {
					...
					const firstComment = await getDoc<'feedbackComment'>(ctx, item.firstCommentId);
...
export const searchForLinking = optionalAuthQuery
	...
	.query(async ({ ctx, input }) => {
		...
		const feedback = input.search.trim()
			? await ctx.db
					.query('feedback')
					.withSearchIndex('by_projectId_boardId_status_searchContent', (q: any) =>
						q
							.search('searchContent', input.search)
							.eq('projectId', asId<'project'>(input.projectId))
					)
