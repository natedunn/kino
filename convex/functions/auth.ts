			...
			organization: {
				change: async (change: any, ctx: any) => {
					...
					if (change.newDoc.slug !== change.oldDoc.slug) {
						...
						const [projects, storageRows, connectionStates, installations, repoConnections] =
							await Promise.all([
								db
									.query('project')
								...
								db
									...
									.withIndex('by_orgId', (q: any) => q.eq('orgId', change.newDoc.id))
									.collect(),
								db
								...
								db
									...
									.withIndex('by_orgId_repoId', (q: any) => q.eq('orgId', change.newDoc.id))
									.collect(),
							]);
			...
			user: {
				create: {
					before: async (data: any, ctx: any) => {
						...
						const resolvedUsername = await ensureUniqueUsername({ db: ctx.db, orm }, usernameValue);
						return {
							data: {
								...data,
