...
export const create = authMutation
	...
	.mutation(async ({ ctx, input }) => {
		...
		const item = await ensureUpdateCommentAccess(ctx, input.updateId, ctx.userId);
...
export const remove = authMutation
	...
	.mutation(async ({ ctx, input }) => {
		...
		// actions.
		await ctx.orm.delete(updateCommentTable).where(eq(updateCommentTable.id, comment._id));
		return { deleted: true };
	});
