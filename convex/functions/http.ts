...
import { getEnv, getTrustedForwardedAuthOrigin, isTrustedOrigin } from '../lib/get-env';
...
app.use(async (c, next) => {
	...
	if (c.req.path.startsWith('/api/auth')) {
		return getAuth(c.env as any).handler(restoreTrustedForwardedAuthHeaders(c.req.raw));
	}
