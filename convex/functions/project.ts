...
const PROJECT_PURGE_BATCH_SIZE = 25
    ...
    if (input.urls !== undefined) {
      ...
      const verifiableUrls = new Set(
        connections
          ...
          .map((connection: any) =>
            normalizeUrl(repoCanonicalUrl(connection.repoFullName))
          )
      )
      ...
      patch.urls = input.urls.map((entry) => {
        ...
        const verified =
          entry.source === "github" && verifiableUrls.has(normalized)
        return {
          source: verified ? "github" : "manual",
          text: entry.text,
          url: entry.url,
          verifiedAt: verified
            ? (existingVerifiedAt.get(normalized) ?? Date.now())
            : null,
        }
      })
    }
...
export const getManyByOrg = optionalAuthQuery
  ...
  .query(async ({ ctx, input }) => {
    ...
    if (!profile) {
      const visible = publicProjects.filter(
        (project: any) => project.deletedTime == null
      )
...
function repoCanonicalUrl(repoFullName: string) {
...
export const purgeProject = internalMutation({
  ...
  handler: async (ctx, { projectId }) => {
    ...
    }> = [
      {
        ...
        remove: (id) =>
          octx.orm
            ...
            .where(eq(feedbackBoardTable.id, id as any)),
      },
      {
        query: () =>
          ctx.db
            ...
            .take(PROJECT_PURGE_BATCH_SIZE),
        remove: (id) =>
          octx.orm.delete(updateTable).where(eq(updateTable.id, id as any)),
      },
      ...
      },
      {
        query: () =>
          ctx.db
            ...
            .take(PROJECT_PURGE_BATCH_SIZE),
        remove: (id) =>
          octx.orm.delete(feedbackTable).where(eq(feedbackTable.id, id as any)),
      },
      {
        query: () =>
          ctx.db
        ...
        remove: (id) =>
          octx.orm
            ...
            .where(eq(projectMemberTable.id, id as any)),
      },
    ]
    ...
    await octx.orm
      .delete(projectTable)
      .where(eq(projectTable.id, projectId as any))
    return null
  },
})
