...
async function counts(ctx: Ctx) {
	const [feedback, comments, emotes, events, upvotes, boards] = await Promise.all([
...
describe('feedback cascade deletes (ORM layer)', () => {
	it('deleting a board cascades to feedback and all its descendants', async () => {
		...
		await t.run(async (baseCtx) => {
			...
			await ctx.orm.delete(feedbackBoardTable).where(eq(feedbackBoardTable.id, board.id));
	...
	it('deleting a feedback cascades to its comments, emotes, events, and upvotes', async () => {
		...
		await t.run(async (baseCtx) => {
			...
			await ctx.orm.delete(feedbackTable).where(eq(feedbackTable.id, feedback.id));
	...
	it('deleting a single comment removes only its emotes, leaving feedback intact', async () => {
		...
		await t.run(async (baseCtx) => {
			...
			await ctx.orm.delete(feedbackCommentTable).where(eq(feedbackCommentTable.id, comment.id));
...
describe('update cascade deletes (ORM layer)', () => {
	async function seedUpdateTree(ctx: Ctx, args: { profileId: string; projectId: string }) {
	...
	it('deleting a single update comment removes only its emotes', async () => {
		...
		await t.run(async (baseCtx) => {
			...
			await ctx.orm.delete(updateCommentTable).where(eq(updateCommentTable.id, comment.id));
