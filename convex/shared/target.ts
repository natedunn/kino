...
export function isValidTarget(target: string, granularity: TargetGranularity) {
	switch (granularity) {
		case 'day':
...
export function resolveTarget(target: string, granularity: TargetGranularity): TargetRange {
	switch (granularity) {
		case 'day': {
		...
		case 'quarter': {
			...
			return {
				end: `${parsed.year}-${pad2(endMonth)}-${pad2(daysInMonth(parsed.year, endMonth))}`,
...
export function formatTarget(target: string, granularity: TargetGranularity) {
	switch (granularity) {
		case 'day': {
...
export function compareTargets(
	...
	return GRANULARITY_SORT_ORDER[left.granularity] - GRANULARITY_SORT_ORDER[right.granularity];
}
