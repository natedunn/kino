...
import { isTrustedTargetUrl, timingSafeEqualString, type GatewayEnv } from "./env"
...
describe("isTrustedTargetUrl", () => {
  it("accepts tier preview, local, and convex site targets", () => {
    ...
    expect(
      isTrustedTargetUrl(
        devEnv,
        "https://neptune.kino.localhost:1355/api/github/callback"
      )
    ).toBe(true)
    expect(
      isTrustedTargetUrl(
        devEnv,
        "https://happy-otter-123.convex.site/api/github/webhook"
      )
    ).toBe(true)
  ...
  it("rejects untrusted origins and protocols", () => {
    expect(isTrustedTargetUrl(devEnv, "https://evil.com/api/github/webhook")).toBe(
      false
    )
    expect(
      isTrustedTargetUrl(
        devEnv,
        "https://kino.hello-fc8.workers.dev.evil.com/cb"
      )
    ).toBe(false)
