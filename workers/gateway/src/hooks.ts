...
async function listTargets(env: GatewayEnv) {
	...
	do {
		...
		const values = await Promise.all(page.keys.map((key) => env.TARGETS.get(key.name, 'json')));
...
async function forwardDelivery(request: Request, body: string, target: RegisteredTarget) {
	...
	try {
		...
		if (!response.ok) {
			console.warn(`webhook forward to ${target.url} failed: ${response.status}`);
		}
	} catch (error) {
		console.warn(
...
export async function handleGitHubWebhook(
	...
	ctx.waitUntil(
		Promise.allSettled(targets.map((target) => forwardDelivery(request, body, target)))
	);

	return new Response(JSON.stringify({ forwardedTo: targets.length, ok: true }), {
