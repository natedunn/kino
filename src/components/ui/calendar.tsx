		...
		<DayPicker
			...
			classNames={{
				button_next: cn(
					...
					'absolute top-1 right-1 size-7 opacity-70 hover:opacity-100'
				),
				button_previous: cn(
					buttonVariants({ size: 'icon-sm', variant: 'ghost' }),
					'absolute top-1 left-1 size-7 opacity-70 hover:opacity-100'
				),
					...
					const Icon =
						orientation === 'left'
							...
							: orientation === 'right'
								? ChevronRight
								: ChevronDown;
					return <Icon aria-hidden='true' className={cn('size-4', chevronClassName)} />;
				},
			}}
