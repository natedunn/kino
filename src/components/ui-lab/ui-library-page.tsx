...
export function UiLibraryPage() {
	const { item } = useSearch({ strict: false }) as UiSearch;
	const navigate = useNavigate({ from: '/ui' });
	...
	const setActiveId = (id: string) => navigate({ search: { item: id }, resetScroll: false });

	const sectionOf = SECTIONS.find((s) => s.items.some((i) => i.id === active.id));
