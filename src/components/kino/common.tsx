...
export function PageFrame({ children, sidebar }: { children: ReactNode; sidebar?: ReactNode }) {
	return (
...
export function StatPill({ label, value }: { label: string; value: ReactNode }) {
	return (
		<div className='rounded-2xl border border-border/70 bg-background/70 px-4 py-3'>
			<div className='text-[11px] tracking-[0.18em] text-muted-foreground uppercase'>{label}</div>
...
export function StatusBadge({
	...
	return (
		<span
			className={cn('inline-flex rounded-full px-2.5 py-1 text-xs font-medium capitalize', tone)}
		>
			{value.replaceAll('-', ' ')}
		</span>
	);
...
export function ProjectTabs({ orgSlug, projectSlug }: { orgSlug: string; projectSlug: string }) {
	const tabClassName =
...
export function PrimaryLinkButton({
	...
	return (
		<Link className={buttonVariants({ size: 'lg' })} params={params as never} to={to as never}>
			{children}
		</Link>
	);
}
