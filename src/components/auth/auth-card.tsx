...
export function AuthFooter(props: { children: ReactNode }) {
	return <div className='mt-6 border-t pt-4 text-sm text-muted-foreground'>{props.children}</div>;
}

/** A labeled input row used across the auth forms. */
export function AuthField(props: { id: string; label: string; children: ReactNode }) {
	return (
