...
function MembersSettingsRoute() {
  ...
  const updateRole = useMutation(
    crpc.orgMember.updateMemberRole.mutationOptions()
  )
  const removeMember = useMutation(
    crpc.orgMember.removeMember.mutationOptions()
  )
  const cancelInvite = useMutation(
    crpc.orgMember.cancelInvitation.mutationOptions()
  )
      ...
      {pendingQuery.data && pendingQuery.data.length > 0 ? (
        <div className="mt-8">
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            Invitations are created but email delivery isn’t configured yet —
            share the invite link manually for now.
          </p>
        </div>
      ) : null}
    </section>
