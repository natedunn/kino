...
function SortHeader({
	...
	return (
		<button
			...
			className='group inline-flex items-center gap-1.5 text-xs font-medium tracking-wider text-muted-foreground uppercase transition-colors hover:text-foreground'
		>
			{children}
...
const columns: Array<ColumnDef<ProjectFile>> = [
				...
				<div className='flex items-center gap-3'>
					<div
						className={cn(
							...
							file.thumbnailUrl ? 'border-border/60 bg-muted/40' : 'border-border/40 bg-muted/60'
						)}
					>
						{file.thumbnailUrl ? (
					...
					<div className='flex min-w-0 flex-col gap-0.5'>
						...
						<span className='font-mono text-[10px] text-muted-foreground/60'>{file.mimeType}</span>
					</div>
				</div>
			);
		},
	...
	},
	{
		...
		cell: ({ row }) => {
			...
			if (size === null) return <span className='text-xs text-muted-foreground/50 italic'>—</span>;
			return <span className='font-mono text-xs text-muted-foreground'>{formatBytes(size)}</span>;
		},
		sortingFn: (a, b) => {
	...
	},
	{
		...
		sortingFn: (a, b) => a.original.uploadedAt.getTime() - b.original.uploadedAt.getTime(),
	},
	{
		id: 'actions',
...
function FilesPage() {
	...
	const filteredData = useMemo(() => {
		return MOCK_FILES.filter((file) => {
			...
			const matchesKind =
				activeKind === 'all' ||
				(activeKind === 'google-drive' ? file.source === 'google-drive' : file.kind === activeKind);
							...
							{table.getHeaderGroups().map((headerGroup) => (
								<tr key={headerGroup.id} className='border-b border-border/50 bg-muted/40'>
									{headerGroup.headers.map((header) => (
										<th key={header.id} className='px-4 py-3 text-left first:pl-5 last:pr-5'>
								...
								table.getRowModel().rows.map((row, i) => (
									<tr
										...
										className={cn(
											...
											i !== table.getRowModel().rows.length - 1 && 'border-b border-border/40'
										)}
									>
										{row.getVisibleCells().map((cell) => (
											<td key={cell.id} className='px-4 py-3 align-middle first:pl-5 last:pr-5'>
					...
					{table.getRowModel().rows.length > 0 && (
						<div className='flex items-center justify-between border-t border-border/40 bg-muted/20 px-5 py-2.5'>
							<p className='font-mono text-[11px] text-muted-foreground/70'>
								...
								of <span className='font-semibold text-foreground'>{MOCK_FILES.length}</span> files
							</p>
