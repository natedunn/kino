...
function ProjectGeneralSettingsRoute() {
  ...
  const importMutation = useMutation(
    crpc.projectExternal.importGithubUrls.mutationOptions()
  )
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value, formApi }) => {
      ...
      const urls = value.urls.filter(
        (entry) => entry.text.trim() || entry.url.trim()
      )
      ...
      try {
        ...
        formApi.reset({
          ...
          visibility: (updated.visibility ??
            value.visibility) as ProjectVisibility,
        })
  ...
  return (
    <section className="max-w-3xl">
      ...
      >
        ...
        >
          <SectionCard
            description="Displayed across Kino on your project pages."
            label="Name"
          >
          ...
          <SectionCard
            description={
              <>
                Website, docs, socials, or anything else. Up to{" "}
                {MAX_PROJECT_URLS}.
              </>
            }
            label="Links"
                      ...
                      {canImport ? (
                            ...
                            try {
                              ...
                              if (
                                !field.state.value.some(
                                  (entry) => entry.url === result.repoUrl
                                )
                              ) {
                                field.insertValue(0, {
                                  source: "github",
                        ...
                        >
                          ...
                          {importMutation.isPending
                            ? "Adding…"
                            : `Add from ${
                                importInfoQuery.data?.repoFullName ?? "GitHub"
                              }`}
                        </Button>
                      ) : null}
                    </div>
          ...
          >
            <form.Field name="visibility">
              {(field) => (
                <Select
                  onValueChange={(value) =>
                    field.handleChange(value as ProjectVisibility)
                  }
                  value={field.state.value}
                >
        ...
        <div className="sticky bottom-0 z-10 flex items-center justify-end gap-3 border-t bg-background/80 py-4 backdrop-blur md:-ml-8 md:w-[calc(100%_+_2rem)]">
          ...
          >
            {({ isSubmitting, name, slug }) => {
              ...
              const cannotSave =
                !isDirty || !name.trim() || !slug.trim() || saving
        ...
        >
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                You have unsaved changes to this project. If you leave now, your
