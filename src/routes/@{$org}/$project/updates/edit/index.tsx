...
function validateDashboardSearch(search: Record<string, unknown>): {
	...
	return {
		pageSize: PAGE_SIZES.has(pageSize) ? (pageSize as 10 | 20 | 50) : DEFAULT_PAGE_SIZE,
	};
}
...
function UpdatesDashboardRoute() {
	...
	const { pageSize } = routeApi.useSearch();
	const crpc = useCRPC();
...
function UpdatesDashboard({ canDelete, pageSize }: { canDelete: boolean; pageSize: number }) {
	const params = routeApi.useParams();
	...
	const sheetUpdate = useMemo(
		() => (sheetUpdateId ? (allRows.find((row) => row.id === sheetUpdateId) ?? null) : null),
	...
	const columns = useMemo(
		() => createUpdateColumns({ org: params.org, project: params.project }, setSheetUpdateId),
	...
	const table = useReactTable({
		...
		pageCount: dashboardQuery.data.isDone ? pagination.pageIndex + 1 : pagination.pageIndex + 2,
		state: {
			pagination,
			rowSelection,
			columnVisibility,
	...
	function handlePageSizeChange(nextPageSize: number) {
		void navigate({
			params: { org: params.org, project: params.project },
			search: {
				pageSize: nextPageSize as 10 | 20 | 50,
			},
	...
	function handleNextPage() {
		if (dashboardQuery.data.isDone || !dashboardQuery.data.continueCursor) return;
										...
										>
											{row.getVisibleCells().map((cell) => {
												...
												return (
													<td
														key={cell.id}
														className={cn('px-3 py-3 align-middle sm:px-4', meta?.cellClassName)}
													>
