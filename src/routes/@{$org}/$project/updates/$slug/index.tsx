...
export const Route = createFileRoute('/@{$org}/$project/updates/$slug/')({
	...
	loader: async ({ context, params }) => {
		...
		const updateData = await context.queryClient.ensureQueryData(
			crpcServer.update.getDetailCritical.queryOptions({
...
function dedupeUpdateComments(comments: Array<UpdateCommentData>) {
...
function UpdateDetailRoute() {
	...
	const commentMutation = useMutation(crpc.updateComment.create.mutationOptions());
	...
	const isAuthenticated = !!currentProfile || auth.hasSession || auth.isAuthenticated;
	...
	async function revalidateMiddleComments() {
		...
		const refreshed: Array<UpdateCommentData> = [];
			...
			<div className='border-b'>
				<div className='container flex items-start gap-4 pt-10 pb-6'>
					...
					<div className='flex shrink-0 items-center gap-1'>
						...
						<Tooltip>
							<TooltipTrigger asChild>
								<Button onClick={handleShareTwitter} size='icon' variant='ghost'>
