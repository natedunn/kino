...
describe("requireAuth", () => {
  ...
  it("lets authenticated visitors through", () => {
    expect(() =>
      requireAuth({ isAuthenticated: true }, { href: "/dashboard" })
    ).not.toThrow()
  })
})
...
describe("auth-snapshot derivations", () => {
  it("deriveClientAuthed fails OPEN while loading", () => {
    expect(
      deriveClientAuthed({ isAuthenticated: false, isLoading: true })
    ).toBe(true)
    expect(
      deriveClientAuthed({ isAuthenticated: false, isLoading: false })
    ).toBe(false)
    expect(
      deriveClientAuthed({ isAuthenticated: true, isLoading: false })
    ).toBe(true)
  })

  it("deriveClientDefinitelyAuthed fails CLOSED while loading", () => {
    expect(
      deriveClientDefinitelyAuthed({ isAuthenticated: true, isLoading: true })
    ).toBe(false)
    expect(
      deriveClientDefinitelyAuthed({ isAuthenticated: true, isLoading: false })
    ).toBe(true)
    expect(
      deriveClientDefinitelyAuthed({ isAuthenticated: false, isLoading: false })
    ).toBe(false)
  })
})
