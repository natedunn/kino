...
export const emailSchema = z.string().trim().toLowerCase().max(FORM_LIMITS.email).email();
...
export const projectUrlSchema = z.object({
	...
	text: z.string().trim().min(1, 'Link label is required').max(FORM_LIMITS.urlLabel),
	url: httpUrlSchema,
});
...
export const feedbackFormSchema = z.object({
	firstComment: z.string().trim().min(1, 'Comment is required').max(FORM_LIMITS.comment),
	title: z.string().trim().min(1, 'Title is required').max(FORM_LIMITS.feedbackTitle),
});
...
export const updateFormSchema = z.object({
	content: z.string().trim().min(1, 'Content is required').max(FORM_LIMITS.updateContent),
	tags: z
		.array(
			z
				.string()
		...
		.max(20),
	title: z.string().trim().min(1, 'Title is required').max(FORM_LIMITS.updateTitle),
});
