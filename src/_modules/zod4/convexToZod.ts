...
import type { Zid } from './id';

import z from 'zod';

import { zid } from './id';

type GetValidatorT<V extends GenericValidator> = V extends Validator<infer T, any, any> ? T : never;
				...
				: V extends VInt64<any, any>
					...
					: V extends VBoolean<any, any>
						...
						: V extends VNull<any, any>
							? z.ZodNull
							: V extends VLiteral<infer T extends string | number | boolean | bigint | null, any>
								? z.ZodLiteral<T>
									...
									: V extends VRecord<any, infer Key, infer Value, any, any>
										? Key extends VId<GenericId<infer TableName>>
											? z.ZodRecord<
												...
												>
											: z.ZodRecord<z.core.$ZodString<'string'>, ZodValidatorFromConvex<Value>>
										: V extends VUnion<
													any,
													[
...
export function convexToZod<V extends GenericValidator>(
	...
	switch (convexValidator.kind) {
		...
		case 'union': {
			...
			const memberValidators = unionValidator.members.map((member: GenericValidator) =>
				convexToZod(member)
			);
		...
		case 'record': {
			zodValidator = z.record(z.string(), convexToZod((convexValidator as any).value));
			break;
		}
		default:
