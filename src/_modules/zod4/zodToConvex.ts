...
import { findBaseCodec } from './codec';
import { isZid, registryHelpers } from './id';

type IsZid<T> = T extends { __tableName: infer _TableName extends string } ? true : false;
										...
										: Z extends z.$ZodUnion<infer T>
											...
											: Z extends z.$ZodLiteral<infer T>
												...
												: Z extends z.$ZodEnum<infer T>
													...
													: Z extends z.$ZodRecord<z.$ZodString<string>, infer V extends z.$ZodType>
														? VRecord<
																...
																string
															>
														: Z extends z.$ZodNullable<infer Inner extends z.$ZodType>
															? Inner extends z.$ZodOptional<infer InnerInner extends z.$ZodType>
																? VOptional<
																		VUnion<
		...
		switch (defType) {
			...
			case 'tuple': {
				...
				if (actualValidator instanceof z.$ZodTuple) {
					const items = (actualValidator as any).def?.items as z.$ZodType[] | undefined;
