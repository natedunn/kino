import type { Customization, Registration } from 'convex-helpers/server/customFunctions';
import type {
	ActionBuilder,
	ArgsArrayToObject,
	FunctionVisibility,
...
import type { PropertyValidators, Value } from 'convex/values';
...
type ReturnValueInput<ReturnsValidator extends z.$ZodType | ZodValidator | void> = [
	ReturnsValidator,
] extends [z.$ZodType]
...
type ReturnValueOutput<ReturnsValidator extends z.$ZodType | ZodValidator | void> = [
	ReturnsValidator,
] extends [z.$ZodType]
...
export function customFnBuilder(
	...
	return function customBuilder(fn: any): any {
		...
		const returnValidator =
			returns && !fn.skipConvexValidation ? { returns: zodOutputToConvex(returns) } : null;
		...
		if (args && !fn.skipConvexValidation) {
			...
			return builder({
				...
				handler: async (ctx: any, allArgs: any) => {
					...
					if (!parsed.success) {
						throw new ConvexError({
							ZodError: JSON.parse(JSON.stringify(parsed.error, null, 2)) as Value[],
						});
					}
